...
app.get("/profile/:id", async (req, res) => {
  ...
  res.render("users/profile", {
    ...
    pdata: pdata,
    member: member,
    bots: await botsdata.find({
      $or: [{ ownerID: member.id }, { coowners: member.id }],
    }),
  });
});
...
// Edit biography
app.post("/profile/:id/edit/bio", rlimit, async (req, res) => {
  try {
    ...
    if (pdata && pdata.userID !== req.user.id)
      return error(res, "You are not authorized to edit this profile.");

    if (biography.length > 100)
      return error(res, "Biography must be less than 100 characters.");
...
app.get("/profile/:id/comments", async (req, res) => {
  ...
  if (res.user && res.user.id !== member.id)
    return res.render("404.ejs", {
      bot: global.client ? global.client : null,
      path: req.path,
      user: req.isAuthenticated() ? req.user : null,
  ...
  let bots = await botsdata
    .find({ rates: { $elemMatch: { author: member.id } } })
    .sort({ "rates.date": -1 });
