...
app.get("/bots/new", async (req, res) => {
  ...
  if (
    (await global.client.users.fetch(req.user.id)).createdTimestamp +
      2592000000 >
    Date.now()
  )
    return res.render("404.ejs", {
...
app.post("/bots/new", async (req, res) => {
  try {
    ...
    let application;
    try {
      application = await global.client.rest.get(
        "/applications/" + appID + "/rpc"
      );
    ...
    if (!prefix || typeof prefix !== "string")
      return error(res, "<strong>[Prefix]</strong> is required.");
    if (!shortDesc || typeof shortDesc !== "string")
      return error(res, "<strong>[Short Description]</strong> is required.");
    if (!longDesc || typeof longDesc !== "string")
      return error(res, "<strong>[Long Description]</strong> is required.");

    if (shortDesc.length < 50 || shortDesc.length > 140)
      return error(
        res,
        "<strong>[Short Description]</strong> must be between 100 and 500 characters."
      );
    if (longDesc.length < 200 || longDesc.length > 5000)
      return error(
        res,
        "<strong>[Long Description]</strong> must be between <strong>500</strong> and <strong>5000</strong> characters."
      );

    if (!tags || typeof tags != "object" || !Array.isArray(tags))
      return error(res, "<strong>[Tags]</strong> is required.");
    if (!tags.every((tag) => config.website.botTags.includes(tag)))
      return error(
        res,
        `<strong>[Tags]</strong> must be one of the following: ${config.website.botTags
          .map((tag) => `<code>${tag}</code>`)
          .join(", ")}`
      );
    if (tags.length < 3)
      return error(res, "<strong>[Tags]</strong> must be at least 3.");
    if (tags.length > 7)
      return error(res, "<strong>[Tags]</strong> you cannot exceed 7 tags.");
    ...
    if (supportURL) {
      try {
        ...
        if (!invite.guild)
          return error(
            res,
            "<strong>[Invite]</strong> is not a valid server invite."
          );
      } catch (e) {
        console.log(e);
        return error(
    ...
    //     webhookURL.indexOf("https://discord.com/api/")
    //   )
    //     return error(
    //       res,
    //       "<strong>[Webhook URL]</strong> is not a valid webhook URL."
    //     );
    //   const segments = webhookURL.split("/").slice(4);
    //   if (segments[0] == "webhooks") segments.unshift("v10");
    //   else if (["v6", "v7", "v8", "v9"].includes(segments[0]))
    //     segments[0] = "v10";
    ...
    //     if (
    //       segments[1] != "webhooks" ||
    //       Number.isNaN(segments[2]) ||
    //       segments[3]
    //         ?.split("")
    //         .find((letter) => base64UrlAlphabet.indexOf(letter) == -1) ||
    //       segments[4] ||
    //       segments.length != 4
    //     )
    //       return error(
    ...
    if (coowners) {
      coowners = coowners.filter(
        (item, index) => coowners.indexOf(item) === index
      );
    ...
    await new botsdata({
      ...
      ownerID: req.user.id,
      avatar:
        botUser.displayAvatarURL({ format: "png", size: 512 }) ??
    ...
    const bot = await global.client.users.cache.get(botID);
    global.client.channels.cache
      .get(global.config.server.channels.bot_add)
      .send({
        embeds: [
          new EmbedBuilder()
            .setColor("Green")
            .setDescription(
              `**@${req.user.username}** submitted [**@${bot.username}**](${global.config.website.url}/bot/${botID})`
            ),
        ],
        content: `<@&${config.server.roles.botReviewer}>`,
      });
  } catch (e) {
    console.log(e.stack);
    return error(
