...
const rlimit = eRatelimit({
  ...
  handler: (req, res, next, options) => {
    error(res, "You can only edit your bot 1 time every 30 seconds");
  },
});

app.post("/bots/edit/:botID", rlimit, async (req, res) => {
  try {
    if (!req.user)
      return res.render("404.ejs", {
        bot: global.client ? global.client : null,
    ...
    return global.client.channels.cache
      .get(global.config.server.channels.bot_edit)
      .send({
        embeds: [
          new EmbedBuilder()
            .setColor("Yellow")
            .setDescription(
              `**@${req.user.username}** edited [**@${botdata.username}**](${global.config.website.url}/bot/${botdata.botID})`
            ),
        ],
      });
  } catch (e) {
